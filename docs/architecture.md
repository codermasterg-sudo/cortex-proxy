...
sequenceDiagram
    ...
    Start->>Proxy: NewProxyServer(client, configMgr, rep)
    Proxy->>Proxy: LoadCA() ← 从磁盘加载自签CA
    Start->>Start: http.ListenAndServe(:7898, proxy)
    Start-->>User: cortex-proxy listening on :7898
```
...
sequenceDiagram
    ...
    alt 压缩未启用 或 非 JSON 请求
        ...
        alt 平台可用
            ...
            Handler->>Handler: 合并原始JSON字段 + 替换 messages
    ...
    GProxy->>GProxy: 存储 record_id 至 ctx.UserData
...
sequenceDiagram
    ...
    LLM-->>GProxy: HTTP 响应
    GProxy->>Response: ExtractAndEnqueueUsage(resp, recordID, fields, rep)

    alt SSE 流式响应 (text/event-stream)
        Response->>Response: 直接透传，不读取 body
    else 普通 JSON 响应
        ...
        Response->>Reporter: Enqueue({record_id, input_tokens, ...})
...
goproxy 在每次 CONNECT 隧道建立时调用 `IssueForHost` 动态签发证书，对每个目标域名都生成独立证书。
...
classDiagram
    class Client {
        ...
        +Compress(ctx, rawBody) CompressResult
...
**超时说明：** `Client` 创建时将 `timeoutMS`（默认 3000）设置为 `http.Client.Timeout`，该超时覆盖所有 API 调用（压缩、获取配置、上报）。
...
classDiagram
    class ProxyServer {
        +NewProxyServer(client, configMgr, rep) *goproxy.ProxyHttpServer
...
**并发保护：** `flushing bool` 标志防止多个 goroutine 同时触发 flush（`Enqueue` 触发的 `go flush()` 与 ticker 触发的可能并发）。
